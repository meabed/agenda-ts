 ...
 * @access private
 */
const processDbResult: ProcessDbResultMethod = async function (this: Agenda, job, result) {
 ...
 */
export const saveJob: SaveJobMethod = async function (this: Agenda, job) {
  try {
    if (!this._collection) {
    ...
    if (props.type === 'single') {
      ...
      const result = await this._collection.findOneAndUpdate(
        ...
        update,
        { includeResultMetadata: true, upsert: true, returnDocument: 'after' }
      );
      return await processDbResult.call(this, job, result);
    }
